...
export class AbstractMove extends Move {
  ...
  constructor(
    ...
    if (this.game.select(source)!.pieceName === 'Pawn' && ['1', '8'].includes(target.charAt(1))) {
      this.promotion = true;
    }
    if (game.abstract === false) {
      this.abstractGame = new Game(this.game.toFen(), true);
      this.abstractGame.takeMove(source, target, new AbstractMove(this.abstractGame, source, target, capture));
