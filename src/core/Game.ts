...
export class Game extends EventEmitter {
  ...
  constructor(fen: string = 'rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1', public abstract = false) {
    super();
    this.reset();
    this.parseFen(fen);
    this.emit('start');
  ...
  public getPositionAttackingMoves(pos: string, color: WBColor) {
    ...
    const attackMoves = pieces.map((piece) => piece.getLegalMoves()).reduce((prev, now) => prev.concat(now), []);
  ...
  public takeMove(source: string, target: string, abstractMove?: AbstractMove) {
    ...
    if (piece.pieceName === 'Pawn' && Math.abs(move.y - piece.position.y) === 2) {
      // enpassant
      const enPassantPos = Point.getNotation(piece.position.x, (piece.position.y + move.y) / 2);
    ...
    const castlingPiece = move.castling instanceof Move ? this.select(move.castling.source) : null;
  ...
  public parseFen(fen: string) {
    this.reset();
    const { pieces, possibleCastlings, turn, enpassant, lastCatchNumber, moveNumber } = parseFen(fen);
  ...
  public iterator(pos: string, color: 'w' | 'b', mode: 'straight' | 'diagonal' | 'Lshape' | 'line', limit = Infinity) {
    const { x, y } = new Point(pos);
    const iterators = [];
    if (mode !== 'line') {
